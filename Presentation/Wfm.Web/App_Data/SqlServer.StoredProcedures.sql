CREATE FUNCTION [dbo].[wfm_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
CREATE FUNCTION [dbo].[wfm_getnotnullnotempty]
(
...
CREATE FUNCTION [dbo].[wfm_getprimarykey_indexname]
(
...
BEGIN
	...
	SELECT CAST(data as int) FROM [wfm_splitstring_to_table](@CategoryIds, ',')	
	...
	SELECT CAST(data as int) FROM [wfm_splitstring_to_table](@FilteredSpecs, ',')
	...
	SELECT CAST(data as int) FROM [wfm_splitstring_to_table](@AllowedCustomerRoleIds, ',')
...
BEGIN
	--create catalog
	EXEC('
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''wfmCommerceFullTextCatalog'')
		CREATE FULLTEXT CATALOG [wfmCommerceFullTextCatalog] AS DEFAULT')
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + dbo.[wfm_getprimarykey_indexname] ('Product') +  '] ON [wfmCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductVariant]''))
		...
		KEY INDEX [' + dbo.[wfm_getprimarykey_indexname] ('ProductVariant') +  '] ON [wfmCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[LocalizedProperty]''))
		...
		KEY INDEX [' + dbo.[wfm_getprimarykey_indexname] ('LocalizedProperty') +  '] ON [wfmCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
		...
		KEY INDEX [' + dbo.[wfm_getprimarykey_indexname] ('ProductTag') +  '] ON [wfmCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
END
GO
...
BEGIN
	...
	--drop catalog
	EXEC('
	IF EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''wfmCommerceFullTextCatalog'')
		DROP FULLTEXT CATALOG [wfmCommerceFullTextCatalog]
	')
END
GO
