...
namespace Wfm.Core.Plugins
{
    ...
    public static class PluginFileParser
    {
        ...
        public static PluginDescriptor ParsePluginDescriptionFile(string filePath)
        {
            ...
            foreach (var setting in settings)
            {
                ...
                switch (key)
                {
                    ...
                    case "FileName":
                        ...
                        break;
                    case "LimitedToFranchises":
                        {
                            //parse list of Franchise IDs
                            foreach (var str1 in value.Split(new [] {','}, StringSplitOptions.RemoveEmptyEntries)
                                                      .Select(x => x.Trim()))
                            {
                                int franchiseId = 0;
                                if (int.TryParse(str1, out franchiseId))
                                {
                                    descriptor.LimitedToFranchises.Add(franchiseId);
                                }
                            }
                        }
                        break;
            ...
            //wfmCommerce 2.00 didn't have 'SupportedVersions' parameter
        ...
        public static void SavePluginDescriptionFile(PluginDescriptor plugin)
        {
            ...
            if (plugin.LimitedToFranchises.Count > 0)
            {
                var franchiseList = "";
                for (int i = 0; i < plugin.LimitedToFranchises.Count; i++)
                {
                    franchiseList += plugin.LimitedToFranchises[i];
                    if (i != plugin.LimitedToFranchises.Count - 1)
                        franchiseList += ",";
                }
                keyValues.Add(new KeyValuePair<string, string>("LimitedToFranchises", franchiseList));
            }
