...
{
    public partial class T4_2016Service : IT4BaseService
    {
        #region Fields

        private const int _TaxYear = 2016;
        ...
        private readonly IRepository<T4_2016> _t4s;
        ...
        public T4_2016Service(
            IWebHelper webHelper,
            IRepository<Franchise> franchises,
            IRepository<T4_2016> t4s,
        ...
        public IQueryable<T4_Base> GetAllT4s(int? franchiseId = null, bool includeSubmitted = true)
        {
            ...
            if (!includeSubmitted)
                result = result.Where(x => x.IsSubmitted == false);
        ...
        public string GetEmployeeT4PDF(int franchiseId, int t4Id, bool isEncrypted, out int candidateId)
        {
            ...
            var pdfTemplate = @"t4-fill-14e.pdf";
